...
rsm_client::primary_failure() {
    ...
    for (std::vector<std::string>::iterator it = known_mems.begin(); it != known_mems.end(); ++it) {
        ...
        if (cl->bind(rpcc::to(1000)) < 0)
            printf("rsm_client::primary_failure cannot bind to %s", (*it).c_str());
        int ret = cl->call(rsm_client_protocol::members, 0, mems, rpcc::to(1000));
        printf("PP:: Received ret=%d from %s\n", ret, (*it).c_str());
        if (ret == rsm_protocol::OK) {
            std::string new_primary = mems.back();
            printf("PP::New Primary: %s\n", new_primary.c_str());
            mems.pop_back();
...
rsm_client::invoke(int proc, std::string req, std::string &rep) {
    ...
    while (1) {
        ...
        primary.nref++;
        assert(pthread_mutex_unlock(&rsm_client_mutex) == 0);
        //printf("rsm_client::failed before rpc\n");
        ret = primary.cl->call(rsm_client_protocol::invoke, proc, req,
                rep, rpcc::to(5000));
        //printf("rsm_client::failed after rpc\n");
        assert(pthread_mutex_lock(&rsm_client_mutex) == 0);
        primary.nref--;
