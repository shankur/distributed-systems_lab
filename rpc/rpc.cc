...
rpcs::add_reply(unsigned int clt_nonce, unsigned int xid,
        char *b, int sz) {
    ScopedLock rwl(&reply_window_m_);
    std::map<unsigned int, std::list<reply_t> >::iterator miter = reply_window_.find(clt_nonce);
    if (miter == reply_window_.end()) {
        std::list<reply_t> lis;
        reply_window_[clt_nonce] = lis;
        struct reply_t X(xid);
        X.cb_present = true;
        X.buf = b;
        X.sz = sz;
        reply_window_[clt_nonce].push_back(X);
    }
    else {
...
rpcs::checkduplicate_and_update(unsigned int clt_nonce, unsigned int xid,
    ...
    ScopedLock rwl(&reply_window_m_);
    std::map<unsigned int, std::list<reply_t> >::iterator mapiter = reply_window_.find(clt_nonce);
    if (mapiter == reply_window_.end()) {
        // NEW Client
        return NEW;
    }
    else {
        ...
        if (xid <= xid_rep) {
            ...
            // Old Client, with requests not replied
                ...
                if (it->xid == xid) {
                    ...
                    if (it->cb_present == true) {
                        ...
                        *sz = (it->sz);
                        ...
                        return INPROGRESS;    // not replied
                    }
                }
            }
            // client is old, but request is new
            return NEW;
        }
    }
}
