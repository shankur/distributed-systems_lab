...
rsm::rsm(std::string _first, std::string _me)
    ...
    pthread_mutex_init(&state_transfer_mutex, NULL);
...
rsm::statetransfer(std::string m) {
    ...
    printf("rsm::statetransfer transfer from %s success, vs(%d,%d)\n",
            m.c_str(), last_myvs.vid, last_myvs.seqno);
    printf("S::%s state transfer complete (in function)\n", cfg->myaddr().c_str());
    return true;
}
...
rsm::statetransferdone(std::string m) {
    ...
    rpcc *cl = h.get_rpcc();
    if (cl == 0)
        printf("rsm::statetransferdone: cannot get socket of primary\n");
    int dummy =0;
    assert(pthread_mutex_unlock(&rsm_mutex) == 0);
    if (cl->call(rsm_protocol::transferdonereq, cfg->myaddr(), dummy, rpcc::to(1000)) == rsm_protocol::OK)
    {
        assert(pthread_mutex_lock(&rsm_mutex) == 0);
        inviewchange = false;
        return true;
...
rsm::client_invoke(int procno, std::string req, std::string &r) {
    ...
    std::vector<std::string> cur_nodes = cfg->get_curview();
    bool leave = false;
    for (std::vector<std::string>::iterator it = cur_nodes.begin(); it != cur_nodes.end(); ++it) {
        if (cfg->myaddr() == *it)
            continue;
        handle h(*it);
        rpcc *cl = h.get_rpcc();
        ...
        int x = 0;
        if (cl->call(rsm_protocol::invoke, procno, v, req, x, rpcc::to(1000)) != rsm_client_protocol::OK) {
            pthread_mutex_lock(&rsm_mutex);
            last_myvs = myvs;
            myvs = v;
            pthread_mutex_unlock(&rsm_mutex);
...
rsm::invoke(int proc, viewstamp vs, std::string req, int &dummy) {
    ...
    if (!(vs > myvs)) {
        pthread_mutex_unlock(&rsm_mutex);
        printf("viewstamp mismatch vs.id=%d vs.seq=%d myvs.id=%d myvs.seq=%d\n", vs.vid, vs.seqno, myvs.vid, myvs.seqno);
...
rsm::transferreq(std::string src, viewstamp last, rsm_protocol::transferres &r) {
    assert(pthread_mutex_lock(&rsm_mutex) == 0);
    int ret = rsm_protocol::OK;
    printf("rsm::statetransfer transferreq from %s (%d,%d) vs (%d,%d)\n", src.c_str(),
            last.vid, last.seqno, last_myvs.vid, last_myvs.seqno);
    if (stf && last != last_myvs) {
        long long l = last.vid;
        l = l << 32;
        l = l | last.seqno;
        pthread_mutex_lock(&state_transfer_mutex);
        std::map<long long, std::string>::iterator it = state_map.find(l);
        if (it != state_map.end())
            r.state = state_map[l];
        else {
            state_map[l] = stf->marshal_state();
            r.state = state_map[l];
        }
        pthread_mutex_unlock(&state_transfer_mutex);
    }
    r.last = last_myvs;
    assert(pthread_mutex_unlock(&rsm_mutex) == 0);
    return ret;
...
rsm::transferdonereq(std::string m, int &r) {
    ...
    assert (pthread_mutex_lock(&rsm_mutex) == 0);
    if (cfg->myaddr() == primary) {
        printf("rsm::state_transfer primary: received transferdone rpc nbackup:%d vec:%d\n", nbackup, transfer_set.size());
        transfer_set.insert(m);
        if (nbackup == transfer_set.size() + 1) {
            transfer_set.clear();
            nbackup = 0;
            inviewchange = false;
